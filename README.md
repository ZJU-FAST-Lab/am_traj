...
- The library implements an extremely fast Minimum Jerk solver when optimal time allocation and constraints are not considered. It uses Banded LU Factorization which is much more efficient here than a general Sparse LU Factorization. The former has linear time complexity while the latter may stuck in its "analyzePattern" for several minutes. In comparison, our lib __only requires 4 seconds__ to generate a __Minimum Jerk Trajectory with 1000000 Segments__!!!
